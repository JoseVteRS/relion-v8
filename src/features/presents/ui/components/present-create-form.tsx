...
import { Euro, Eye, EyeOff, Gift, Link2, ListIcon, Loader2, Sparkles, X } from "lucide-react";
import { useState } from "react";
import { useForm } from "react-hook-form";
import z from "zod";
import { presentCreateSchema } from "../../schemas/present-schemas";
import { ListSearchCommand } from "./list-search-command";

type FormValues = z.infer<typeof presentCreateSchema>;

type PresentData = FormValues;

interface PresentFormProps {
  onSubmit: (data: PresentData) => void;
  onCancel?: () => void;
  isLoading?: boolean;
  initialValues?: Partial<FormValues> & { listName?: string };
}
...
export const PresentCreateForm = ({
  ...
  const form = useForm<FormValues>({
    resolver: zodResolver(presentCreateSchema),
    defaultValues: {
  ...
  const handleClearList = () => {
    form.setValue("listId", undefined);
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button 
              type="submit" 
              disabled={isLoading}
              className="flex-1 sm:flex-none sm:min-w-[140px] bg-emerald-600 hover:bg-emerald-700 text-white"
            >
              {isLoading ? (
                <>
