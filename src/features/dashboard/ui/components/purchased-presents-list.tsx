...
interface PurchasedPresentsListProps {
  picks: PickData[];
}

export function PurchasedPresentsList({ picks }: PurchasedPresentsListProps) {
  if (picks.length === 0) {
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            No has comprado ningún regalo todavía
          </CardDescription>
        </CardHeader>
      </Card>
    );
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              Últimos {picks.length} regalo{picks.length !== 1 ? "s" : ""} que has comprado
            </CardDescription>
          </div>
